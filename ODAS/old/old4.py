...
class CLS:
	def __init__(self, port=18888):
		...
		self.label = ['nonspeech' for i in range(4)]
	...
	async def handle(self, client_socket, loop):
		...
		while True:
			...
			async with self.lock:
				if len(detected) != 0:
					self.label = [i['category'] for i in detected[-1]['src']]
...
class SSS_CLS:
	def __init__(self):
		...
		# self.active = [[] for i in range(TMP)]
		# self.active_updated_time = None
	...
	def update_audio(self, raw):
		# self.active_updated_time = time.time()
		for i in range(TMP):
			try:
				...
				# self.active[i].append(np.max(input_audio) >= self.volume_threshold)
	...
	def infer(self):
		...
		for i in range(TMP):
			if label_txt[i] not in self.whitelist:
				label_txt[i] = 'silence'
		# if time.time() - self.active_updated_time >= self.check_duration:
		# 	for i in range(TMP):
		# 		if sum(self.active[i]) <= len(self.active[i]) / 2:
		# 			label_txt[i], acc[i] = 'dead', 100.0
		# 		self.active[i] = []
		# 		self.active_updated_time = time.time()
			...
			async with self.lock:
				for now in detected:
					for ind, target in enumerate(now['src']):
						...
						if target['id'] == 0:
							continue
						# if target['h'] < 0.2 or target['h'] > 0.8:
						# 	continue
...
angles = [(i * 22.5, (i + 1) * 22.5) for i in range(0, 16)]
...
class WS:
	async def doa(self, websocket):
		while True:
			try:
				...
				async with self.classify_odas.lock:
					label_odas = self.classify_odas.label
				async with self.classify.lock:
					label, acc = self.classify.label, self.classify.acc
				for i in range(len(pos)):
					# if label_odas[i] == 'speech':
					# 	pos[i]['category'] = 'Speech'
					# else:
				...
				# 		ri, rj = pos[i]['angle'] // 22.5, pos[j]['angle'] // 22.5
				# 		if ri == rj:
				# 			if pos[i]['category'] == 'silence' and pos[j]['category'] != 'silence':
				# 				pos[i]['category'] = 'dead'
				# 			else:
				# 				pos[j]['category'] = 'dead'
				pos = [i for i in pos if i['category'] != 'dead']
	...
	async def odas(self, sst, ssl, classify_odas, loop):
		...
		odas_cls_task = asyncio.create_task(classify_odas.listen(loop))

		await asyncio.gather(sst_task, sst_audio_task, ssl_task, odas_cls_task)
	...
	async def announce(self):
		...
		while True:
			try:
				ip = netifaces.ifaddresses(get_connection_name_from_guid()['WLAN'])[AF_INET][0]['addr']
				break
			except Exception as e:
	...
	async def main(self):
		...
		self.classify_odas = CLS()
		self.sst = SSS()
		self.ssl = SSL()
		self.port = 11451

		odas_task = asyncio.create_task(self.odas(self.sst, self.ssl, self.classify_odas, loop))
