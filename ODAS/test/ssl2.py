				...
				try:
					for target in data['src']:
						...
						if (target_x in range(-40, 40) and target_y in range(-40, 40)):
							continue
						with echo_lock:
							if target['id'] in now_pos and target['activity'] < 0.2:
								last_activity = now_pos[target['id']]['last_activity']
							else:
								last_activity = time.time()
							now_pos[target['id']] = {'x': target_x + 400, 'y': target_y + 400, 'activity': target['activity'], 'last_activity': last_activity, 'energy': energy}
...
class WindowThread(threading.Thread):
	def __init__(self):
		threading.Thread.__init__(self)
	def run(self):
		global running, echo_lock, now_x, now_y
		while running == True:
