				...
				try:
					for target in data['src']:
						...
						if (target_x in range(-40, 40) and target_y in range(-40, 40)) or energy < 100:
							continue
						with echo_lock:
							pos.append({'x': target_x + 400, 'y': target_y + 400, 'energy': energy})
	...
	def run(self):
		...
		while running == True:
			...
			with echo_lock:
				try:
					for i in pos:
						now_x, now_y, now_energy = i['x'], i['y'], i['energy']
						cv2.circle(img, (now_x, now_y), 30, (spectrum_rgb3_lut[255- now_energy][0], spectrum_rgb3_lut[255- now_energy][1], spectrum_rgb3_lut[255- now_energy][2]), -1)
					cv2.imshow('pu', img)
